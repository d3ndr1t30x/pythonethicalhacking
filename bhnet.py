...
#this runs a command and returns the ouput 
def run_command(cmd):
    #trim the newline
    cmd = cmd.rstrip()
...
def client_sender(buffer):
    ...
    try:
        ...
        #if no we are going to wait for the user to punch some in 
        if len(buffer):
            client.send(buffer.encode('utf-8'))

        while True:
            #now we wait for the data back
            recv_len = 1
            response = b''
        ...
        #just catch eneric errors - you can do your homework to beef this up
